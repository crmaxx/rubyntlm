module Net
  module NTLM
    ...
    class Message < FieldSet
      class << Message
        def parse(str)
          ...
          when 3
            ...
            fail ArgumentError, "unknown type: #{m.type}"
          end
          t
        end
      ...
      def serialize
        deflag
        super + security_buffers.map { |_, f| f.value }.join
      end
      ...
      def security_buffers
        @alist.find_all { |_, f| f.instance_of?(SecurityBuffer) }
      end
      ...
      def data_edge
        security_buffers.map { |_, f| f.active ? f.offset : size }.min
      end
    end
  end
end
